 ...
 if test x"$enable_image" != xno; then
  ...
  if test x"$x11" = xyes; then
   if test x"$have_gtk2" = xyes; then
     ...
     AC_DEFINE(USE_GTK2)
     IMGX11CFLAGS="`${PKG_CONFIG} --cflags gdk-pixbuf-2.0 gdk-pixbuf-xlib-2.0 gtk+-2.0 x11`"
     IMGX11LDFLAGS="`${PKG_CONFIG} --libs gdk-pixbuf-2.0 gdk-pixbuf-xlib-2.0 gtk+-2.0 x11`"
   elif test x"$have_gdkpixbuf" = xyes; then
     AC_DEFINE(USE_W3MIMG_X11)
     IMGOBJS="$IMGOBJS x11/x11_w3mimg.o"
     IMGTARGETS="x11"    
     AC_DEFINE(USE_GDKPIXBUF)
     IMGX11CFLAGS="`${GDKPIXBUF_CONFIG} --cflags x11`"
     IMGX11LDFLAGS="`${GDKPIXBUF_CONFIG} --libs x11` -lgdk_pixbuf_xlib"
   elif test x"$have_imlib" = xyes; then
     AC_DEFINE(USE_W3MIMG_X11)
     IMGOBJS="$IMGOBJS x11/x11_w3mimg.o"
     IMGTARGETS="x11"    
     AC_DEFINE(USE_IMLIB)
     IMGX11CFLAGS="`${IMLIB_CONFIG} --cflags x11`"
     IMGX11LDFLAGS="`${IMLIB_CONFIG} --libs x11`"
     IMGTARGETS="x11"    
     ...
     IMGX11CFLAGS="`${IMLIB2_CONFIG} --cflags x11`"
     IMGX11LDFLAGS="`${IMLIB2_CONFIG} --libs x11`"
   else
